...
ObjetoVenda.SalvarVenda = function (venda) {
    ...
    $.ajax({
        type: 'POST',
        url: "api/AdicionarProdutos",
        datatype: "JSON",
        ContentType: "application/json",
        cache: false,
        async: true,
        data: {
            "id": venda, "nome": nome, "quantidade": qtd
        },
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach(function (entry) {
                ...
                var idQuantidade = "qtd_" + entry.id;

                htmlConteudo += "<label id='" + idNome + "' > Produto: " + entry.nomeProduto + "</label></br>";
                htmlConteudo += "<label>  Valor: " + entry.preco + "</label></br>";

                htmlConteudo += "<input type'number' id='" + idQuantidade + "'>";

                htmlConteudo += "<input type='button' onclick='ObjetoVenda.SalvarVenda(" + entry.id + ")' value ='Compras'> </br> ";
